    ...
    public class SplitButton : Button
    {
        ...
        public ContextMenuStrip? SplitMenuStrip
        {
            ...
            set
            {
                ...
                {
                    splitMenuStrip.Closing -= OnSplitMenuStripClosing;
                    splitMenuStrip.Opening -= OnSplitMenuStripOpening;
                }
                ...
                {
                    ShowSplit = true;
                    value.Closing += OnSplitMenuStripClosing;
                    value.Opening += OnSplitMenuStripOpening;
                }
                else
                    ShowSplit = false;
        ...
        protected override void OnGotFocus(EventArgs e)
        {
            ...
            if (!State.Equals(PushButtonState.Pressed)
                && !State.Equals(PushButtonState.Disabled))
            {
        ...
        protected override void OnKeyDown(KeyEventArgs kevent)
        {
            if (showSplit)
            {
                ...
                else if (kevent.KeyCode.Equals(Keys.Space)
        ...
        protected override void OnMouseUp(MouseEventArgs mouseEvent)
        {
            ...
            if (mouseEvent.Button == MouseButtons.Right
                && ClientRectangle.Contains(mouseEvent.Location)
                && !isSplitMenuVisible)
            {
                ShowContextMenuStrip();
            }
                ...
                && !isSplitMenuVisible)
            {
                ...
                if (ClientRectangle.Contains(mouseEvent.Location)
                    && !dropDownRectangle.Contains(mouseEvent.Location))
                {
        ...
        protected override void OnPaint(PaintEventArgs paintEvent)
        {
            ...
            if (State != PushButtonState.Pressed
                && IsDefault && !Application.RenderWithVisualStyles)
            {
                var backgroundBounds = bounds;
                backgroundBounds.Inflate(-1, -1);
                ButtonRenderer.DrawButton(graphics, backgroundBounds, State);

                // button renderer doesnt draw the black frame when themes are off
                graphics.DrawRectangle(SystemPens.WindowFrame, 0, 0, bounds.Width - 1, bounds.Height - 1);
            }
            else
            {
            ...
            var drawSplitLine = (State == PushButtonState.Hot
                || State == PushButtonState.Pressed
            ...
            if (RightToLeft == RightToLeft.Yes)
            {
                ...
                if (drawSplitLine)
                {
                    ...
                    graphics.DrawLine(SystemPens.ButtonShadow, bounds.Left + SplitSectionWidth, BorderSize, bounds.Left + SplitSectionWidth, bounds.Bottom - BorderSize);
                    graphics.DrawLine(SystemPens.ButtonFace, bounds.Left + SplitSectionWidth + 1, BorderSize, bounds.Left + SplitSectionWidth + 1, bounds.Bottom - BorderSize);
                }
            }
            else
            {
                ...
                {
                    // draw two lines at the edge of the dropdown button
                    graphics.DrawLine(SystemPens.ButtonShadow, bounds.Right - SplitSectionWidth, BorderSize, bounds.Right - SplitSectionWidth, bounds.Bottom - BorderSize);
                    graphics.DrawLine(SystemPens.ButtonFace, bounds.Right - SplitSectionWidth - 1, BorderSize, bounds.Right - SplitSectionWidth - 1, bounds.Bottom - BorderSize);
                }
            }
            ...
            PaintTextandImage(graphics, new Rectangle(0, 0, ClientRectangle.Width - SplitSectionWidth, ClientRectangle.Height));
        ...
        private void OnSplitMenuPopup(object sender, EventArgs e)
        {
            isSplitMenuVisible = true;
        }

        private void OnSplitMenuStripClosing(object? sender, ToolStripDropDownClosingEventArgs e)
        {
            isSplitMenuVisible = false;
        ...
        private void OnSplitMenuStripOpening(object? sender, CancelEventArgs e)
        {
            isSplitMenuVisible = true;
        }
        ...
        private static HorizontalAlignment GetHorizontalAlignment(ContentAlignment align)
        {
            return align switch
            {
                ContentAlignment.BottomLeft or ContentAlignment.MiddleLeft or ContentAlignment.TopLeft => HorizontalAlignment.Left,
                ContentAlignment.BottomCenter or ContentAlignment.MiddleCenter or ContentAlignment.TopCenter => HorizontalAlignment.Center,
                ContentAlignment.BottomRight or ContentAlignment.MiddleRight or ContentAlignment.TopRight => HorizontalAlignment.Right,
        ...
        private static VerticalAlignment GetVerticalAlignment(ContentAlignment align)
        {
            return align switch
            {
                ...
                ContentAlignment.MiddleLeft or ContentAlignment.MiddleCenter or ContentAlignment.MiddleRight => VerticalAlignment.Center,
